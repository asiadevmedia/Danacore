 ...
 */
  ...
  public static function generateLoginUrl($state = null)
  {
    ...
    $url = sprintf('%s/d/portal/oauth?clientId=%s&scopes=%s&requestId=%s&state=%s&terminalType=%s&redirectUrl=%s',
      Config::getWebUrl(),
      Config::$clientId,
      Config::$oauthScopes,
      Util::generateGuid(),
  ...
  public static function generateSeamlessLoginUrl($phoneNo, $userId, $verifiedTime, $state = '')
  {
    ...
    $seamlessData  = json_encode($preferences, JSON_UNESCAPED_SLASHES);
    $seamlessSign = Util::generateSignature($seamlessData, Config::$privateKey);

    $url = sprintf('%s/m/portal/oauth?seamlessData=%s&seamlessSign=%s&clientId=%s&scopes=%s&requestId=%s&state=%s&terminalType=%s&redirectUrl=%s',
      Config::getWebUrl(),
      URLEncode($seamlessData),
      URLEncode($seamlessSign),
      Config::$clientId,
      Config::$oauthScopes,
      Util::generateGuid(),
      $state,
